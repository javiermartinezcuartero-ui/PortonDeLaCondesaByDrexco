...
- **Pipeline con transiciones válidas.** `LeadRequestStatus` es un enum plano; la máquina de estados (qué transición es válida desde cada estado, `WON` terminal, `LOST` solo reabre a `NURTURING`, `LOST` exige `lostReason`) vive en `changeLeadRequestStatus` (`lib/domain/lead-requests.ts`), no en el esquema.
